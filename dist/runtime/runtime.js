...
class Runtime {
    ...
    deserializeAccountArgs(args) {
        ...
        return {
            ...returnValue,
            ...Object.fromEntries([...encodedArgs.entries()].map(([argName, accountShortName]) => [
                argName,
    ...
    async run(fn, args) {
        ...
        try {
            ...
            if (args) {
                utils_2.debug(`Passing ${Object.getOwnPropertyNames(args)}`);
            }
            await fn(this.deserializeAccountArgs(args), this);
        }
        catch (error) {
            console.error(error.stack);
            throw error; // TODO Figure out better error handling
        }
        finally {
    ...
    async createAndDeploy(name, wasm) {
        ...
        const contractData = await node_fs_1.promises.readFile(wasm);
        const result = await account.najAccount.deployContract(contractData);
        utils_2.debug(`deployed contract ${wasm} to account ${name} with result ${JSON.stringify(result)} `);
...
class TestnetRuntime extends Runtime {
    ...
    get keyFilePath() {
        return node_path_1.join(os.homedir(), '.near-credentials', 'testnet', `${this.masterAccount}.json`);
    }
    async getKeyStore() {
        const keyStore = new nearAPI.keyStores.UnencryptedFileSystemKeyStore(node_path_1.join(os.homedir(), '.near-credentials'));
        return keyStore;
    }
    serializeAccountArgs(args) {
    ...
    async beforeConnect() {
        ...
        utils_2.debug(`Added masterAccount ${this.config.masterAccount} with keyStore ${this.keyStore} and publicKey ${await this.keyStore.getKey(this.config.network, this.masterAccount)}
    ...
    async createAccount(name, keyPair) {
        ...
        utils_2.debug(`New Account: https://explorer.testnet.near.org/accounts/${account.accountId}`);
        return account;
    }
    async createAndDeploy(name, wasm) {
        // TODO: dev deploy!!
        const account = await super.createAndDeploy(name, wasm);
        utils_2.debug(`Deployed new account: ${this.config.explorerUrl}/accounts/${account.accountId}`);
        return account;
    }
    async ensureKeyFileFolder() {
        const keyFolder = node_path_1.dirname(this.keyFilePath);
        try {
            await node_fs_1.promises.mkdir(keyFolder, { recursive: true });
        }
        catch {
            // TODO: check error
        }
...
class SandboxRuntime extends Runtime {
    ...
    get keyFilePath() {
        return node_path_1.join(this.homeDir, 'validator_key.json');
    }
    async getKeyStore() {
