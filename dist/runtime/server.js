    ...
    return new Promise(resolve => {
        const request = http.request(options, result => {
            if (result.statusCode === 200) {
                ...
                internal_utils_1.debug(`Sandbox running but got non-200 response: ${JSON.stringify(result)}`);
                resolve(false);
            }
        });
        request.on('error', error => {
            internal_utils_1.debug(JSON.stringify(error));
            resolve(false);
        });
        // Write data to request body
        request.write(pollData);
        internal_utils_1.debug(`polling server at port ${options.port}`);
        request.end();
    });
}
...
class SandboxServer {
    ...
    static async init(config) {
        await internal_utils_1.ensureBinary();
        const server = new SandboxServer(config);
        if (server.config.refDir) {
            await internal_utils_1.rm(server.homeDir);
            await internal_utils_1.copyDir(server.config.refDir, server.config.homeDir);
        }
        if ((await internal_utils_1.exists(server.homeDir)) && server.config.init) {
            await internal_utils_1.rm(server.homeDir);
        }
        if (server.config.init) {
            try {
                const { stderr, code } = await server.spawn('init');
                internal_utils_1.debug(stderr);
                if (code && code < 0) {
        ...
        internal_utils_1.debug('created ' + server.homeDir);
        return server;
    }
    get homeDir() {
    ...
    async start() {
        ...
        internal_utils_1.debug(`sending args, ${args.join(' ')}`);
        if (process_1.default.env.NEAR_RUNNER_DEBUG) {
            const filePath = path_1.join(this.homeDir, 'sandboxServer.log');
            internal_utils_1.debug(`near-sandbox logs writing to file: ${filePath}`);
            this.subprocess = internal_utils_1.spawn(internal_utils_1.sandboxBinary(), args, {
        ...
        this.subprocess.on('exit', () => {
            internal_utils_1.debug(`Server with port ${this.port}: Died ${this.readyToDie ? 'gracefully' : 'horribly'}`);
        });
        await sandboxStarted(this.port);
        internal_utils_1.debug(`Connected to server at ${this.internalRpcAddr}`);
        return this;
    }
    async close() {
        var _a;
        ...
        if (this.config.rm) {
            await internal_utils_1.rm(this.homeDir);
        }
    }
    get internalRpcAddr() {
    ...
    async spawn(command) {
        return internal_utils_1.asyncSpawn('--home', this.homeDir, command);
    }
}
